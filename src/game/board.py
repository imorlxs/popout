...
class Board:
    ...
    def can_pop(self, col: int, player: int) -> bool:
        ...
        return 0 <= col < COLS and self.grid[ROWS - 1][col] == player
    ...
    def get_winner(self) -> int:
        ...
        for row in range(ROWS):
            w = self._check_line(self.grid[row])
            if w:
                return w

        # Vertical
        for col in range(COLS):
            w = self._check_line([self.grid[row][col] for row in range(ROWS)])
            if w:
                return w
        ...
        for row in range(ROWS - 3):
            for col in range(COLS - 3):
                w = self._check_line(
                    [self.grid[row + k][col + k] for k in range(4)]
                )
                if w:
                    return w
