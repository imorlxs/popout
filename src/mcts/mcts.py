"""
Monte Carlo Tree Search (MCTS) with UCT for the PopOut game.

Algorithm overview
------------------
1. **Selection**   – traverse the tree using UCT until a node that is not
                     fully expanded (or terminal) is reached.
2. **Expansion**   – add one (or more) child nodes for unexplored moves.
3. **Simulation**  – play a random rollout from the new node to a terminal
                     state.
4. **Backpropagation** – propagate the result back up the tree, updating
                          visit counts and win scores.

Variants explored
-----------------
* ``max_children``          – limits expansion width per node.
* ``rollout_policy``        – 'random' (default) or 'heuristic'.
* ``exploration_constant``  – UCT C parameter (default √2 ≈ 1.414).
"""
...
from ..game.board import Board, PLAYER1, PLAYER2, EMPTY, ROWS, COLS
